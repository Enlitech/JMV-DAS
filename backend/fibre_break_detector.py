...
class FibreBreakDetector:
    ...
    def __init__(self, spatial_ewma_alpha: float = 0.2, threshold: float = 1000.0, min_length_m: float = 100.0):
        ...
        self._emv_profile = np.empty(0, dtype=np.float32)
    ...
    def update(self, block: np.ndarray, scale_down: int) -> FibreBreakResult:
        ...
        self._update_emv(profile)

        first_high_pos = self._find_first_high_from_end(self._emv_profile, self.threshold)
        if first_high_pos >= 0:
    ...
    def _update_emv(self, src: np.ndarray):
        src = np.asarray(src, dtype=np.float32).reshape(-1)
        if src.size != self._emv_profile.size:
            self._emv_profile = src.copy()
            return
        ...
        self._emv_profile[0] = src[0]
        for idx in range(1, src.size):
            self._emv_profile[idx] = alpha * src[idx] + (1.0 - alpha) * self._emv_profile[idx - 1]
